...
head('[9b] 📐 줄별 위치·정렬(v0.5.41) — 모델 · 규칙 · 배선');
{
  ...
  const APP = read('renderer/src/App.jsx');
  ok(/menu === 'format' && \(\s*<CaptionToolbar/.test(APP) && /active=\{!!capSel\}/.test(APP), '🔑 툴바는 「서식」 메뉴 리본에 있다(고른 게 없으면 active=false — 안내만)');
  ok(/pickMenu\('format'\)/.test(APP), '🧭 줄 번호·글자 드래그를 하면 서식 메뉴가 열린다');
