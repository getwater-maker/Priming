...
class FlowAutomator {
  ...
  async run(config) {
    ...
    // SRT 자막 생성 — skipVrew(이미지만) 모드에선 생략 (우리 앱이 별도로 만듦)
    if (!config.skipVrew) this._generateSRT(paragraphs, path.join(subDir, 'subtitles.srt'));
    ...
    // 번역 텍스트 저장 — skipVrew(이미지만) 모드에선 생략
    if (!config.skipVrew) {
      let transTxt = '번역 결과\n' + '='.repeat(50) + '\n\n';
