...
- ⏳ **로이가 결정할 일 — 서버의 바이트 동일 사본 24개 정리**: 이 PC 프리셋의 srv: 참조 5건은 전부 원본 이름이라
  안전하지만, **아내 PC 채널이 `_2` 이름을 골랐는지 확인 후** 지워야 한다(앱이 임의 삭제하지 않음).
  이름 같은 스타일 2개(「따듯한 회화풍 일러스트」)도 🎨 에서 하나 지우면 서버에 전파된다.
