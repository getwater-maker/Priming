...
- 🌏 **일본어·베트남어 지원**(2026-09-26 조사, 코드 대기): OmniVoice 는 ja·vi 지원(베트남어는 `language` 지정 필수 · 성조 원어민 검수 필요). 막히는 곳 — 문장분리기 필터(한글·영문 없는 문장 제거 → 일본어 전멸) · 자막 줄 나누기(띄어쓰기 기반 → 일본어 불가) · 글자수 세기(베트남어 성조 글자 미포함) · 전각 ！？ · `~`→「에서」 · 채널 언어 선택지 · 업로드 언어 ko 고정 · 한글 없는 글꼴 제외 · 화자 이름 정규식. ⛔ **한국어 경로는 무변경** — 채널 언어가 ko 가 아닐 때만 분기. 콘텐츠·카테고리 판단은 아도나이로이 「다음 작업」 세션에 인계됨.
