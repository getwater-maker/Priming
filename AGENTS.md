...
- **이미지 프롬프트 내보내기/가져오기/API**(`core/prompt-io.js`, IPC export-prompts/import-prompts/
  ...
  3편 한 번에. 안전치환(PROMPT_SAFE_RULES) 포함. API는 secret-store 키(gemini/Codex/openai).
...
- `vrew/vrew-builder.js` — .vrew 4.0.1 생성 (복사). 형식 변경 시 PrimingFlow AGENTS.md의
...
## 구현 순서 (계획서: ~/.Codex/plans/...0611-glistening-lagoon.md)
0. 이 AGENTS.md 작성 ✅
