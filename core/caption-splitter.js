...
const AUX_FORMS = new Set([
  ...
  '치운', '치울', '치워', '내린', '내릴', '내려',
  // -던 형(과거 회상) — `같은 도시에 와 | 있던` 을 못 막던 구멍(실측)
  '있던', '없던', '보던', '주던', '하던', '되던', '가던', '오던', '두던', '놓던',
  '지던', '버리던', '드리던', '있을', '없을', '있었다', '없었다',
]);
...
// ⑤ 부정부사 — "안/못 + 용언"은 한 덩어리다("꿈쩍도 안 | 하는" 금지).
//    ⚠ '안'은 명사(내부)로도 쓰이지만, 금지의 대가는 「끊지 않는 것」뿐이라 오탐이 해롭지 않다.
//      (쉼표 뒤 `대성당 안, | 자기가` 는 위 쉼표 검사가 먼저 잡아 정당 처리된다.)
const NEG_ADVERBS = new Set(['안', '못']);

// ⑥ 수사 + 단위명사 — "관직 네 | 자리" 금지. 단위명사는 일반명사와 충돌이 적은 것만 골랐다
//    ('장·대·배·점·자·말' 등은 뺐다 — `네 말과 같이` 처럼 단위가 아닌 쓰임이 흔하다).
const RE_NUMERAL = /^(한|두|세|네|다섯|여섯|일곱|여덟|아홉|열|스물|서른|마흔|쉰|예순|일흔|여든|아흔|백|천|만|억|몇|여러|수십|수백|수천|수만|수십만|수백만|[0-9]+)$/;
const RE_UNIT = /^(살|명|개|번|년|해|달|권|마리|척|칸|평|원|차례|가지|종류|시간|세기|켤레|쌍|잔|병|그릇|끼|겹|톨|알|사람|글자|자리|줄기|갈래|덩어리)/;

// ⑦ 부사 — 뒤 용언을 수식하면 붙는 게 낫지만, 문장 전체를 수식하면 끊어도 자연스럽다.
//    표면형으로는 구분이 안 되므로 **약한 감점**만 준다(다른 자리가 있을 때만 피하고 강제하지 않는다).
//    🔑 이게 없으면 이 부사들이 오히려 우대받는다 — '바로·서로·스스로'는 `로$`, '함께'는 `께$` 라
//      아래 RE_GOOD3(부사격조사)에 걸려 **+3점**을 받고 있었다(실측 493건의 원인).
const ADVERBS = new Set([
  '아주', '매우', '너무', '정말', '참', '꼭', '반드시', '전혀', '결코', '절대', '거의',
  '조금', '약간', '많이', '빨리', '천천히', '다시', '이미', '아직', '벌써', '곧', '늘',
  '항상', '자주', '가끔', '특히', '바로', '함께', '서로', '먼저', '겨우', '오직', '단지',
  '그냥', '문득', '슬쩍', '불쑥', '스스로', '도무지', '차마', '제발', '또', '채', '잘',
  '자꾸', '점점', '계속', '대뜸', '얼른', '어서', '한참', '실컷', '몹시', '무척',
]);
const ADVERB_SCORE = -3;
...
function boundaryAt(words, i) {
  ...
  if (NEG_ADVERBS.has(c)) return { banned: true, score: 0, why: `부정부사 「${c} ${n}」` };
  if (RE_NUMERAL.test(c) && RE_UNIT.test(n)) {
    return { banned: true, score: 0, why: `수사+단위 「${c} ${n}」` };
  }
  if (looksAdnominal(c) && RE_CLEAR_JOSA.test(n)) {
    return { banned: false, score: ADNOMINAL_SCORE, why: `관형절 「${c} ${n}」` };
  }
  // ⚠ 부사는 RE_GOOD3(`로$`·`께$`)보다 **먼저** 봐야 한다 — 안 그러면 감점 대신 +3 우대를 받는다.
  if (ADVERBS.has(c)) return { banned: false, score: ADVERB_SCORE, why: `부사 「${c} ${n}」` };
...
function auditCaptionLines(lines) {
  ...
  for (let i = 0; i < lines.length - 1; i++) {
    ...
    // banned = 절대 끊지 말아야 할 자리(0 이어야 정상) / score<0 = 감점 자리(피할 수 있으면 피한 것,
    //   다른 선택지가 없으면 남는다 — 남아 있어도 결함이 아니다). 세는 쪽이 반드시 구분할 것.
    if (b.banned || b.score < 0) {
      bad.push({ i, why: b.why, a: lines[i], b: lines[i + 1], banned: !!b.banned });
    }
  }
  return bad;
}
